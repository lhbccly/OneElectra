    ...
    <section id="story" className="relative overflow-hidden border-t border-line bg-graphite py-20 md:py-28">
      <Container className="relative z-[1] grid items-center gap-10 lg:grid-cols-2 lg:gap-14">
        ...
        >
          ...
          <p className="max-w-2xl text-base leading-relaxed text-muted md:text-lg">{brandStory.body}</p>

          <div className="grid gap-4 sm:grid-cols-3">
            {brandStory.points.map((point, index) => (
              <motion.div
                key={point.title}
              ...
              >
                ...
                <p className="mt-2 text-sm font-medium leading-relaxed text-muted">{point.body}</p>
              </motion.div>
            ))}
          </div>
          ...
          >
            {brandStory.quickFacts.map((item) => (
